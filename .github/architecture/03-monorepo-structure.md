...
easyink/
├── packages/
│   ├── shared/                 # @hcxz/shared — 通用类型、工具、常量
│   ├── schema/                 # @hcxz/schema — Schema 类型、默认值、迁移、序列化、校验
│   ├── core/                   # @hcxz/core — 命令、选择、几何、分页、字体、物料协议
│   ├── datasource/             # @hcxz/datasource — 字段树、数据源引用、绑定格式模板
│   ├── viewer/                 # @hcxz/viewer — ViewerRuntime、预览、打印、导出、缩略图
│   ├── export/
│   │   ├── runtime/            # @hcxz/export-runtime — 导出任务状态机、插件注册（不绑定具体格式实现）
│   │   └── plugin/
│   │       └── dom-pdf/        # @hcxz/export-plugin-dom-pdf — 浏览器 DOM 截图转 PDF 插件（html2canvas + jsPDF）
│   ├── print/
│   │   ├── core/              # @hcxz/print-core — 打印驱动共享纯工具
│   │   ├── integration-easyink-printer/ # @hcxz/print-integration-easyink-printer — EasyInk.Printer 客户端与 Viewer 驱动
│   │   └── integration-hiprint/         # @hcxz/print-integration-hiprint — HiPrint 客户端与 Viewer 驱动
│   ├── builtin/                # @hcxz/builtin — 内置物料注册、Designer/Viewer/Assistant 共享清单
│   ├── designer/               # @hcxz/designer — 设计器工作台 Vue 组件
│   ├── locales/                # @hcxz/locales — 设计器内置语言包
│   ├── prop-schemas/           # @hcxz/prop-schemas — 内置物料基础属性 Schema
│   ├── ui/                     # @hcxz/ui — 面板、表单、工作台基础组件
│   ├── icons/                  # @hcxz/icons — 图标资产
│   ├── schema-tools/           # @hcxz/schema-tools — Schema 校验、DataSource 对齐
│   ├── samples/                # @hcxz/samples — 示例 schema、data 与 datasource
│   ├── assistant/              # @hcxz/assistant-* — Assistant 平台包族
│   └── materials/
│       ├── text/
│       ├── image/
│       ├── barcode/
...
│       ├── chart/
│       ├── table/
│       │   ├── data/          # @hcxz/material-table-data — 动态数据表格物料
│       │   ├── static/        # @hcxz/material-table-static — 静态单元格表格物料
│       │   └── kernel/        # @hcxz/material-table-kernel — 表格共享几何、渲染、编辑与测量工具
│       └── svg/
├── playground/
```
...
- 管理设计态工作台状态
- 通过插槽、`setupStore` 和 `useDesignerStore()` 暴露宿主集成点，不在包级直接依赖 `@hcxz/viewer`
- 默认注册内置物料；宿主通过 `setupStore` 追加注册时以后注册覆盖默认注册
- 依赖 `@hcxz/locales` 和 `@hcxz/prop-schemas` 获取内置语言包与基础属性 Schema；对外仍通过 `@hcxz/designer/locale` 透出语言包
- 暴露 **Contribution API**（`Contribution` / `ContributionRegistry`）作为唯一外部扩展协议；不直接依赖 Assistant 集成包或 `@hcxz/schema-tools`
...
- 作为静态资源包被 `@hcxz/designer` 消费；宿主应用推荐继续从 `@hcxz/designer/locale` 引入，避免把资源包路径变成应用层长期合同
...
- 只依赖 `@hcxz/core` 的 `PropSchema` 类型，不反向依赖 `@hcxz/designer`
...
- `locales` 与 `prop-schemas` 是 Designer 静态资源边界包：前者维护内置语言包，后者维护基础属性 Schema。它们可独立构建、测试和发布，但应用层稳定入口仍优先使用 `@hcxz/designer` 及其 `./locale` facade

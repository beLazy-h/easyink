...
1. **Overlay 几何与物料内容长期错位**。旧设计允许物料用设计态预览层扩大外框，导致 overlay / 吸附 / 框选与 schema `width / height` 分裂；缩放/旋转后坐标系更容易错位。新协议要求元素外框统一以 schema 尺寸为准，物料内部预览行、虚拟列、滚动视口等差异由 `Geometry.resolveLocation / hitTest` 在元素内部消化。
...
`packages/materials/table/kernel/src/*` 中的表格编辑实现全部迁移到新协议：

...
describe('condition field picker', () => {
  it('selects a root field by click', async () => {
    ...
    expect(customerNode!.getAttribute('title')).toBe('customer/name')
    expect(customerNode!.querySelector('.ei-field-node__label')?.getAttribute('title')).toBe('customer/name')
    customerNode!.click()
    await nextTick()
  ...
  it('selects collection child fields as full paths', async () => {
    ...
    button('.condition-field-picker__trigger').click()
    await nextTick()
    expect(document.body.textContent).not.toContain('items/price')
    const nodes = Array.from(document.body.querySelectorAll<HTMLElement>('.ei-field-node__row'))
    const priceNode = nodes.find(node => node.textContent?.includes('Price'))
    expect(priceNode).toBeTruthy()
    expect(priceNode!.getAttribute('title')).toBe('items/price')
    expect(priceNode!.querySelector('.ei-field-node__label')?.getAttribute('title')).toBe('items/price')
    priceNode!.click()
    await nextTick()
  ...
  it('derives full paths for nested fields without explicit paths', async () => {
    ...
    expect(nameNode!.getAttribute('title')).toBe('customer/name')
    expect(nameNode!.querySelector('.ei-field-node__label')?.getAttribute('title')).toBe('customer/name')
    nameNode!.click()
    await nextTick()
